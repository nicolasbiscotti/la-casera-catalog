...
function formatPriceForHistory(price: Price): string {
  if (price.type === 'unit') return `${formatCurrency(price.price)}/${price.unitLabel}`;
  if (price.type === 'weight') return `${formatCurrency(price.pricePerKg)}/kg`;
  if (price.type === 'fraction') return `${formatCurrency(price.prices.quarter || 0)} (1/4)`;
  return '-';
}
...
export function renderExportPage(showToast: (message: string, type?: 'success' | 'error') => void): string {
...
export function attachExportListeners(showToast: (message: string, type?: 'success' | 'error') => void): void {
